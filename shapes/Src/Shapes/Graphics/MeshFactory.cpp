...
namespace Shapes
{
    namespace Graphics
    {
        ...
        Mesh* CreateQuad()
        {
            ...
            SharedPtr<Shapes::Maths::BoundingBox> BoundingBox = CreateSharedPtr<Shapes::Maths::BoundingBox>();
            for(int i = 0; i < 4; i++)
            {
                BoundingBox->Merge(data[i].Position);
            }
    ...
    Graphics::Mesh* Graphics::CreatePrimative(PrimitiveType type)
    {
        ...
        case Graphics::PrimitiveType::File:
            SHAPES_LOG_WARN("Trying to create primitive of type File");
            return nullptr;
        }

        SHAPES_LOG_ERROR("Primitive not supported");
        return nullptr;
    };
