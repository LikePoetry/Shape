...
namespace Shapes
{
    ...
    SceneManager::~SceneManager()
    {
        ...
        if(m_CurrentScene)
        {
            SHAPES_LOG_INFO("[SceneManager] - Exiting scene : {0}", m_CurrentScene->GetSceneName());
    ...
    void SceneManager::SwitchScene(const std::string& name)
    {
        ...
        {
            SHAPES_LOG_ERROR("[SceneManager] - Unknown Scene Alias : {0}", name.c_str());
        }
    }
    ...
    void SceneManager::ApplySceneSwitch()
    {
        ...
        {
            SHAPES_LOG_ERROR("[SceneManager] - Invalid Scene Index : {0}", m_QueuedSceneIndex);
            m_QueuedSceneIndex = 0;
        }
        ...
        {
            SHAPES_LOG_INFO("[SceneManager] - Exiting scene : {0}", m_CurrentScene->GetSceneName());
            app.GetSystem<ShapesPhysicsEngine>()->SetPaused(true);
        ...
        app.GetSystem<ShapesPhysicsEngine>()->SetDefaults();
        app.GetSystem<B2PhysicsEngine>()->SetDefaults();
        app.GetSystem<ShapesPhysicsEngine>()->SetPaused(false);

        std::string physicalPath;
        if(Shapes::VFS::Get().ResolvePhysicalPath("//Scenes/" + m_CurrentScene->GetSceneName() + ".lsn", physicalPath))
        {
        ...
        SHAPES_LOG_INFO("[SceneManager] - Scene switched to : {0}", m_CurrentScene->GetSceneName().c_str());
    ...
    void SceneManager::EnqueueScene(Scene* scene)
    {
        ...
        SHAPES_LOG_INFO("[SceneManager] - Enqueued scene : {0}", scene->GetSceneName().c_str());
    }
