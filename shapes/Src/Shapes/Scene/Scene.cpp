...
namespace Shapes
{
    ...
    void Scene::OnInit()
    {
        ...
        // Physics setup
        auto physics3DSytem = Application::Get().GetSystem<ShapesPhysicsEngine>();
...
#define ALL_COMPONENTSV8 ALL_COMPONENTSV7, TextComponent
    ...
    {
        SHAPES_PROFILE_FUNCTION();
        SHAPES_LOG_INFO("Scene saved - {0}", filePath);
    ...
    void Scene::Deserialise(const std::string& filePath, bool binary)
    {
        ...
        if(binary)
        {
            ...
            {
                SHAPES_LOG_ERROR("No saved scene file found {0}", path);
                return;
            }
            ...
            {
                SHAPES_LOG_ERROR("Failed to load scene - {0}", path);
            }
        }
        else
        {
