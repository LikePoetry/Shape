...
namespace Shapes
{
    ...
    private:
        ...
        {
            SHAPES_ASSERT(ptr, "Creating shared ptr with nullptr");
    ...
    template <class T>
    class SHAPES_EXPORT WeakReference
    {
    public:
...
{
    template <typename T>
    struct hash<Shapes::Reference<T>>
    {
        size_t operator()(const Shapes::Reference<T>& x) const
        {
            return hash<T*>()(x.get());
        }
    };
