...
namespace Shapes
{
	...
	{
        if(!m_Enabled) 
			SHAPES_LOG_ERROR("Option {0} is not provided. Type --help or -h for help", m_LongName);
        
        if(m_ArgValue.empty())
			SHAPES_LOG_ERROR("Option {0} does not have an argument. Type --help or -h for help", m_LongName);
    }
    ...
    {
        m_Description = "Shapes Command Line Parser";
    }
    ...
    void CommandLine::Parse(int argc, char** argv)
    {
        ...
        while(i < argc)
        {
            ...
            if(!foundArgument)
            {
                SHAPES_LOG_ERROR("Ignoring unknown command line argument {0}", flag);
