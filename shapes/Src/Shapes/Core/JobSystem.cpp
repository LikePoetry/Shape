    ...
    namespace System
    {
        ...
        namespace JobSystem
        {
            ...
            void OnInit(uint32_t maxThreadCount)
            {
                ...
                internal_state.numThreads = Shapes::Maths::Min(maxThreadCount, Shapes::Maths::Max(1u, internal_state.numCores - 1));
                ...
                for(uint32_t threadID = 0; threadID < internal_state.numThreads; ++threadID)
                {
                    std::thread worker([threadID]
                            ...
                            SHAPES_PROFILE_SETTHREADNAME(ss.str().c_str());
                    ...
                    SHAPES_ASSERT(affinity_result > 0, "");

                    // Increase thread priority:
                    // BOOL priority_result = SetThreadPriority(handle, THREAD_PRIORITY_HIGHEST);
                    // SHAPES_ASSERT(priority_result != 0, "");
...
#define handle_error_en(en, msg) \
    do {                         \
        ...
        perror(msg);             \
                    ...
                    SHAPES_PROFILE_SETTHREADNAME(wss.str().c_str());
#endif

                    worker.detach();
                }

                SHAPES_LOG_INFO("Initialised JobSystem with [{0} cores] [{1} threads]", internal_state.numCores, internal_state.numThreads);
            }
