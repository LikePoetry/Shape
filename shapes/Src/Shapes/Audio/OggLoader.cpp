...
namespace Shapes
{
    AudioData LoadOgg(const std::string& fileName)
    {
        ...
        if(!Shapes::VFS::Get().ResolvePhysicalPath(fileName, physicalPath))
        {
            SHAPES_LOG_INFO("Failed to load Ogg file : File Not Found");
        }
        ...
        if(!m_FileHandle)
        {
            SHAPES_LOG_CRITICAL("Failed to load OGG file '{0}'!", physicalPath);
        }
        int error;
        ...
        if(!m_StreamHandle)
        {
            SHAPES_LOG_CRITICAL("Failed to load OGG file '{0}'! , Error {1}", physicalPath, error);
            return data;
        }
