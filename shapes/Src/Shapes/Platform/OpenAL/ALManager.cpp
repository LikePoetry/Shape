...
namespace Shapes
{
    namespace Audio
    {
        ...
        void ALManager::OnInit()
        {
            ...
            if(!m_Device)
                SHAPES_LOG_INFO("Failed to Initialise AudioManager! (No valid device!)");

            alcMakeContextCurrent(m_Context);
            alDistanceModel(AL_LINEAR_DISTANCE_CLAMPED);

            SHAPES_LOG_INFO("Initialised AudioManager - {0}", alcGetString(m_Device, ALC_DEVICE_SPECIFIER));
        }
