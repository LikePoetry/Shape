...
namespace Shapes
{
    namespace Graphics
    {
        ...
        void VKRenderer::OnResize(uint32_t width, uint32_t height)
        {
            ...
            SHAPES_LOG_INFO("VKRenderer::OnResize {0}, {1}", width, height);
        ...
        void VKRenderer::BindDescriptorSetsInternal(Graphics::Pipeline* pipeline, Graphics::CommandBuffer* commandBuffer, uint32_t dynamicOffset, Graphics::DescriptorSet** descriptorSets, uint32_t descriptorCount)
        {
            ...
            for(uint32_t i = 0; i < descriptorCount; i++)
            {
                if(descriptorSets[i])
                {
                    ...
                    SHAPES_ASSERT(vkDesSet->GetHasUpdated(Renderer::GetMainSwapChain()->GetCurrentBufferIndex()), "Descriptor Set has not been updated before");
                    numDesciptorSets++;
                }
            }
