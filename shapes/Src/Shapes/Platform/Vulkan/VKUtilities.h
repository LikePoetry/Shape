...
#define VK_CHECK_RESULT(f)                                                                                                                        \
    {                                                                                                                                             \
        ...
        if(res != VK_SUCCESS)                                                                                                                     \
        {                                                                                                                                         \
            SHAPES_LOG_ERROR("[VULKAN] : VkResult is {0} in {1} at line {2}", Shapes::Graphics::VKUtilities::ErrorString(res), __FILE__, __LINE__); \
        }                                                                                                                                         \
    }
...
namespace Shapes
{
    namespace Graphics
    {
        namespace VKUtilities
        {
            ...
            VkPolygonMode PolygonModeToVk(Shapes::Graphics::PolygonMode mode);
            VkPrimitiveTopology DrawTypeToVk(Shapes::Graphics::DrawType type);
        }
    }
}
