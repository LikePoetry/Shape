...
namespace Shapes
{
    namespace Graphics
    {
        ...
        VKPhysicalDevice::VKPhysicalDevice()
        {
            ...
            if(m_GPUCount == 0)
            {
                SHAPES_LOG_CRITICAL("No GPUs found!");
            }
            ...
            if(desiredGPUIndex >= 0)
            {
                ...
                {
                    SHAPES_LOG_CRITICAL("GPU index greater than GPU count!");
                }
                else
                {
            ...
            SHAPES_LOG_INFO("Vulkan : {0}.{1}.{2}", VK_API_VERSION_MAJOR(m_PhysicalDeviceProperties.apiVersion), VK_API_VERSION_MINOR(m_PhysicalDeviceProperties.apiVersion), VK_API_VERSION_PATCH(m_PhysicalDeviceProperties.apiVersion));
            SHAPES_LOG_INFO("GPU : {0}", m_DeviceInfo.Name);
            SHAPES_LOG_INFO("Memory : {0} mb", StringUtilities::ToString(m_DeviceInfo.Memory));
            SHAPES_LOG_INFO("Vendor : {0}", m_DeviceInfo.Vendor);
            SHAPES_LOG_INFO("Vendor ID : {0}", StringUtilities::ToString(m_DeviceInfo.VendorID));
            SHAPES_LOG_INFO("Device Type : {0}", std::string(PhysicalDeviceTypeToString(m_DeviceInfo.Type)));
            SHAPES_LOG_INFO("Driver Version : {0}", m_DeviceInfo.Driver);
            SHAPES_LOG_INFO("APi Version : {0}", m_DeviceInfo.APIVersion);
            ...
            SHAPES_ASSERT(queueFamilyCount > 0, "");
            ...
            if(extCount > 0)
            {
                ...
                {
                    SHAPES_LOG_INFO("Selected physical device has {0} extensions", extensions.size());
        ...
        uint32_t VKPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
        {
            ...
            SHAPES_ASSERT(false, "Could not find a suitable memory type!");
            return UINT32_MAX;
        }
...
#endif
#if defined(SHAPES_PROFILE) && defined(TRACY_ENABLE)
            for(int i = 0; i < 3; i++)
        ...
        bool VKDevice::Init()
        {
...
#if defined(SHAPES_PLATFORM_MACOS) || defined(SHAPES_PLATFORM_IOS)
            ...
            {
                SHAPES_LOG_CRITICAL("[VULKAN] vkCreateDevice() failed!");
                return false;
            }
            ...
            {
                SHAPES_LOG_CRITICAL("[VULKAN] Failed to create VMA allocator");
            }
#endif
