...
namespace Shapes
{
    namespace Graphics
    {
        ...
        const std::vector<const char*> VKContext::GetRequiredExtensions()
        {
            ...
            {
                SHAPES_LOG_INFO("Vulkan : Enabled Validation Layers");
...
#if 0
#if defined(TRACY_ENABLE) && defined(SHAPES_PLATFORM_WINDOWS)
                                          ...
                                          void* userData)
        {
            ...
            {
                SHAPES_LOG_WARN("[VULKAN] - ERROR : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            // Warnings may hint at unexpected / non-spec API usage
            if(flags & VK_DEBUG_REPORT_WARNING_BIT_EXT)
            {
                SHAPES_LOG_WARN("[VULKAN] - WARNING : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            // May indicate sub-optimal usage of the API
            if(flags & VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT)
            {
                SHAPES_LOG_WARN("[VULKAN] - PERFORMANCE : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            // Informal messages that may become handy during debugging
            if(flags & VK_DEBUG_REPORT_INFORMATION_BIT_EXT)
            {
                SHAPES_LOG_WARN("[VULKAN] - INFO : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            // Diagnostic info from the Vulkan loader and layers
            // Usually not helpful in terms of API usage, but may help to debug layer and loader problems
            if(flags & VK_DEBUG_REPORT_DEBUG_BIT_EXT)
            {
                SHAPES_LOG_INFO("[VULKAN] - DEBUG : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            ...
            validationLayers.erase(
                std::remove_if(
                    ...
                    [&](const char* layerName)
                    {
                        ...
                        {
                            removedLayer = true;
                            SHAPES_LOG_WARN("[VULKAN] Layer not supported - {0}", layerName);
                        }
                ...
                std::remove_if(
                    ...
                    [&](const char* extensionName)
                    {
                        ...
                        if(!extensionFound)
                        {
                            ...
                            SHAPES_LOG_WARN("[VULKAN] Extension not supported - {0}", extensionName);
                        }
        ...
        void VKContext::CreateInstance()
        {
            ...
            {
                SHAPES_LOG_CRITICAL("volkInitialize failed");
            }

            if(volkGetInstanceVersion() == 0)
            {
                SHAPES_LOG_CRITICAL("Could not find loader");
            }
#endif
            ...
            {
                SHAPES_LOG_CRITICAL("[VULKAN] Validation layers requested, but not available!");
            }

            if(!CheckExtensionSupport(m_InstanceExtensionNames))
            {
                SHAPES_LOG_CRITICAL("[VULKAN] Extensions requested are not available!");
            }
            ...
            if(sdkVersion > driverVersion)
            {
                ...
                SHAPES_LOG_WARN("Using Vulkan {0}. Please update your graphics drivers to support Vulkan {1}.", driverVersionStr, sdkVersionStr);
            }

            appInfo.sType              = VK_STRUCTURE_TYPE_APPLICATION_INFO;
            appInfo.pApplicationName   = "Runtime";
            appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
            appInfo.pEngineName        = "Shapes";
            appInfo.engineVersion      = VK_MAKE_VERSION(ShapesVersion.major, ShapesVersion.minor, ShapesVersion.patch);
            ...
            if(createResult != VK_SUCCESS)
            {
                ...
                SHAPES_LOG_CRITICAL("[VULKAN] Failed to create instance!");
            }
#ifndef SHAPES_PLATFORM_IOS
        ...
        void VKContext::SetupDebugCallback()
        {
            ...
            {
                SHAPES_LOG_CRITICAL("[VULKAN] Failed to set up debug callback!");
            }
        }
