...
namespace Shapes
{
    namespace Graphics
    {
        ...
        bool VKSwapChain::Init(bool vsync)
        {
            ...
            if(!m_Surface)
            {
                SHAPES_LOG_CRITICAL("[VULKAN] Failed to create window surface!");
            }
            ...
            if(queueIndexSupported == VK_FALSE)
                SHAPES_LOG_ERROR("Present Queue not supported");
        ...
        {
            SHAPES_ASSERT(m_CurrentBuffer < m_SwapChainBufferCount, "Incorrect swapchain buffer index");
        ...
        void VKSwapChain::AcquireNextImage()
        {
            ...
            {
                SHAPES_PROFILE_SCOPE("vkAcquireNextImageKHR");
                ...
                {
                    SHAPES_LOG_INFO("Acquire Image result : {0}", result == VK_ERROR_OUT_OF_DATE_KHR ? "Out of Date" : "SubOptimal");
                ...
                {
                    SHAPES_LOG_CRITICAL("[VULKAN] Failed to acquire swap chain image! - {0}", VKUtilities::ErrorString(result));
                }
            }
        }
        ...
        void VKSwapChain::Present(VkSemaphore semaphore)
        {
            ...
            {
                SHAPES_LOG_ERROR("[Vulkan] SwapChain out of date");
            }
            else if(error == VK_SUBOPTIMAL_KHR)
            {
                SHAPES_LOG_ERROR("[Vulkan] SwapChain suboptimal");
            }
            else
            {
