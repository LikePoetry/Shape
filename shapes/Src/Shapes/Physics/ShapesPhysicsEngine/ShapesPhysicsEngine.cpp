...
namespace Shapes
{
    ...
    float ShapesPhysicsEngine::s_UpdateTimestep = 1.0f / 60.0f;

    ShapesPhysicsEngine::ShapesPhysicsEngine()
        : m_IsPaused(true)
        , m_UpdateAccum(0.0f)
    ...
    void ShapesPhysicsEngine::OnUpdate(const TimeStep& timeStep, Scene* scene)
    {
        ...
        if(!m_IsPaused)
        {
            ...
            {
                SHAPES_PROFILE_SCOPE("Physics::Get Rigid Bodies");
            ...
            {
                SHAPES_PROFILE_SCOPE("Physics::Get Spring Constraints");
                m_Constraints.clear();
            ...
            {
                SHAPES_PROFILE_SCOPE("Physics::Get Axis Constraints");
            ...
            {
                SHAPES_PROFILE_SCOPE("Physics::Get Distance Constraints");
            ...
            {
                SHAPES_PROFILE_SCOPE("Physics::Get Weld Constraints");
            ...
            {
                SHAPES_PROFILE_SCOPE("Physics::UpdatePhysics");
                ...
                {
                    SHAPES_LOG_WARN("Physics too slow to run in real time!");
    ...
    void ShapesPhysicsEngine::UpdatePhysics()
    {
        m_Manifolds.clear();
    ...
    void ShapesPhysicsEngine::UpdateRigidBodys()
    {
        SHAPES_PROFILE_SCOPE("Update Rigid Body");
        for(size_t i = 0; i < m_RigidBodys.size(); i++)
            UpdateRigidBody(m_RigidBodys[i]);
    }

    void ShapesPhysicsEngine::SyncTransforms(Scene* scene)
    {
        SHAPES_PROFILE_FUNCTION();
    ...
    void ShapesPhysicsEngine::UpdateRigidBody(RigidBody3D* obj) const
    {
        SHAPES_PROFILE_FUNCTION();
    ...
    void ShapesPhysicsEngine::BroadPhaseCollisions()
    {
        SHAPES_PROFILE_FUNCTION();
        ...
        SHAPES_LOG_INFO(duplicatePairs);
#endif
    }

    void ShapesPhysicsEngine::NarrowPhaseCollisions()
    {
        SHAPES_PROFILE_FUNCTION();
    ...
    void ShapesPhysicsEngine::SolveConstraints()
    {
        SHAPES_PROFILE_FUNCTION();

        {
            SHAPES_PROFILE_SCOPE("Solve Manifolds");

            for(Manifold& m : m_Manifolds)
                m.PreSolverStep(s_UpdateTimestep);
        }
        {
            SHAPES_PROFILE_SCOPE("Solve Constraints");
        ...
        {
            SHAPES_PROFILE_SCOPE("Apply Impulses");
    ...
    void ShapesPhysicsEngine::ClearConstraints()
    {
        m_Constraints.clear();
    }

    std::string ShapesPhysicsEngine::IntegrationTypeToString(IntegrationType type)
    {
        switch(type)
        {
        case IntegrationType::EXPLICIT_EULER:
    ...
    std::string ShapesPhysicsEngine::BroadphaseTypeToString(BroadphaseType type)
    {
        switch(type)
        {
    ...
    void ShapesPhysicsEngine::SetBroadphaseType(BroadphaseType type)
    {
        ...
        {
        case BroadphaseType::BRUTE_FORCE:
            m_BroadphaseDetection = Shapes::CreateSharedPtr<BruteForceBroadphase>();
            break;
        case BroadphaseType::SORT_AND_SWEAP:
            m_BroadphaseDetection = Shapes::CreateSharedPtr<SortAndSweepBroadphase>();
            break;
        case BroadphaseType::OCTREE:
            m_BroadphaseDetection = Shapes::CreateSharedPtr<OctreeBroadphase>(5, 5, Shapes::CreateSharedPtr<BruteForceBroadphase>());
            break;
        default:
            m_BroadphaseDetection = Shapes::CreateSharedPtr<BruteForceBroadphase>();
            break;
    ...
    void ShapesPhysicsEngine::OnImGui()
    {
        SHAPES_PROFILE_FUNCTION();
    ...
    void ShapesPhysicsEngine::OnDebugDraw()
    {
        SHAPES_PROFILE_FUNCTION();
