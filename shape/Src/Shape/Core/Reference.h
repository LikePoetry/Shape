...
namespace Shape
{
    ...
    template <class T>
    class SHAPE_EXPORT WeakReference
    {
    public:
...
{
    template <typename T>
    struct hash<Shape::Reference<T>>
    {
        size_t operator()(const Shape::Reference<T>& x) const
        {
            return hash<T*>()(x.get());
        }
    };
