...
namespace Shape
{
	...
	void Input::Reset()
	{
		...
		memset(m_MouseClicked, 0, MAX_KEYS);
		memset(m_MouseHeld, 0, MAX_KEYS);
    ...
    {
        SetKeyPressed(Shape::InputCode::Key(e.GetKeyCode()), e.GetRepeatCount() < 1);
        SetKeyHeld(Shape::InputCode::Key(e.GetKeyCode()), true);
        return false;
    }

    bool Input::OnKeyReleased(KeyReleasedEvent& e)
    {
        SetKeyPressed(Shape::InputCode::Key(e.GetKeyCode()), false);
        SetKeyHeld(Shape::InputCode::Key(e.GetKeyCode()), false);
        return false;
    }

    bool Input::OnMousePressed(MouseButtonPressedEvent& e)
    {
        SetMouseClicked(Shape::InputCode::MouseKey(e.GetMouseButton()), true);
        SetMouseHeld(Shape::InputCode::MouseKey(e.GetMouseButton()), true);
        return false;
    }

    bool Input::OnMouseReleased(MouseButtonReleasedEvent& e)
    {
        SetMouseClicked(Shape::InputCode::MouseKey(e.GetMouseButton()), false);
        SetMouseHeld(Shape::InputCode::MouseKey(e.GetMouseButton()), false);
        return false;
    }
