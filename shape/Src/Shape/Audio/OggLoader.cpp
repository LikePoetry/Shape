...
namespace Shape
{
    AudioData LoadOgg(const std::string& fileName)
    {
        ...
        if(!Shape::VFS::Get().ResolvePhysicalPath(fileName, physicalPath))
        {
            SHAPE_LOG_INFO("Failed to load Ogg file : File Not Found");
        }
        ...
        if(!m_FileHandle)
        {
            SHAPE_LOG_CRITICAL("Failed to load OGG file '{0}'!", physicalPath);
        }
        int error;
        ...
        if(!m_StreamHandle)
        {
            SHAPE_LOG_CRITICAL("Failed to load OGG file '{0}'! , Error {1}", physicalPath, error);
            return data;
        }
