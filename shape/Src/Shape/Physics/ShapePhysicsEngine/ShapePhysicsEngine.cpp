...
namespace Shape
{
    ...
    float ShapePhysicsEngine::s_UpdateTimestep = 1.0f / 60.0f;

    ShapePhysicsEngine::ShapePhysicsEngine()
        : m_IsPaused(true)
        , m_UpdateAccum(0.0f)
    ...
    void ShapePhysicsEngine::OnUpdate(const TimeStep& timeStep, Scene* scene)
    {
        ...
        if(!m_IsPaused)
        {
            ...
            {
                SHAPE_PROFILE_SCOPE("Physics::Get Rigid Bodies");
            ...
            {
                SHAPE_PROFILE_SCOPE("Physics::Get Spring Constraints");
                m_Constraints.clear();
            ...
            {
                SHAPE_PROFILE_SCOPE("Physics::Get Axis Constraints");
            ...
            {
                SHAPE_PROFILE_SCOPE("Physics::Get Distance Constraints");
            ...
            {
                SHAPE_PROFILE_SCOPE("Physics::Get Weld Constraints");
            ...
            {
                SHAPE_PROFILE_SCOPE("Physics::UpdatePhysics");
                ...
                {
                    SHAPE_LOG_WARN("Physics too slow to run in real time!");
    ...
    void ShapePhysicsEngine::UpdatePhysics()
    {
        m_Manifolds.clear();
    ...
    void ShapePhysicsEngine::UpdateRigidBodys()
    {
        SHAPE_PROFILE_SCOPE("Update Rigid Body");
        for(size_t i = 0; i < m_RigidBodys.size(); i++)
            UpdateRigidBody(m_RigidBodys[i]);
    }

    void ShapePhysicsEngine::SyncTransforms(Scene* scene)
    {
        SHAPE_PROFILE_FUNCTION();
    ...
    void ShapePhysicsEngine::UpdateRigidBody(RigidBody3D* obj) const
    {
        SHAPE_PROFILE_FUNCTION();
    ...
    void ShapePhysicsEngine::BroadPhaseCollisions()
    {
        SHAPE_PROFILE_FUNCTION();
        ...
        SHAPE_LOG_INFO(duplicatePairs);
#endif
    }

    void ShapePhysicsEngine::NarrowPhaseCollisions()
    {
        SHAPE_PROFILE_FUNCTION();
    ...
    void ShapePhysicsEngine::SolveConstraints()
    {
        SHAPE_PROFILE_FUNCTION();

        {
            SHAPE_PROFILE_SCOPE("Solve Manifolds");

            for(Manifold& m : m_Manifolds)
                m.PreSolverStep(s_UpdateTimestep);
        }
        {
            SHAPE_PROFILE_SCOPE("Solve Constraints");
        ...
        {
            SHAPE_PROFILE_SCOPE("Apply Impulses");
    ...
    void ShapePhysicsEngine::ClearConstraints()
    {
        m_Constraints.clear();
    }

    std::string ShapePhysicsEngine::IntegrationTypeToString(IntegrationType type)
    {
        switch(type)
        {
        case IntegrationType::EXPLICIT_EULER:
    ...
    std::string ShapePhysicsEngine::BroadphaseTypeToString(BroadphaseType type)
    {
        switch(type)
        {
    ...
    void ShapePhysicsEngine::SetBroadphaseType(BroadphaseType type)
    {
        ...
        {
        case BroadphaseType::BRUTE_FORCE:
            m_BroadphaseDetection = Shape::CreateSharedPtr<BruteForceBroadphase>();
            break;
        case BroadphaseType::SORT_AND_SWEAP:
            m_BroadphaseDetection = Shape::CreateSharedPtr<SortAndSweepBroadphase>();
            break;
        case BroadphaseType::OCTREE:
            m_BroadphaseDetection = Shape::CreateSharedPtr<OctreeBroadphase>(5, 5, Shape::CreateSharedPtr<BruteForceBroadphase>());
            break;
        default:
            m_BroadphaseDetection = Shape::CreateSharedPtr<BruteForceBroadphase>();
            break;
    ...
    void ShapePhysicsEngine::OnImGui()
    {
        SHAPE_PROFILE_FUNCTION();
    ...
    void ShapePhysicsEngine::OnDebugDraw()
    {
        SHAPE_PROFILE_FUNCTION();
