...
namespace Shape
{
	namespace Graphics
	{
		...
		VKPhysicalDevice::VKPhysicalDevice()
		{
			...
			if (m_GPUCount == 0)
			{
				SHAPE_LOG_CRITICAL("No GPUs found!");
			}
			...
			if (desiredGPUIndex >= 0)
			{
				...
				{
					SHAPE_LOG_CRITICAL("GPU index greater than GPU count!");
				}
				else
				{
			...
			SHAPE_LOG_INFO("Vulkan : {0}.{1}.{2}", VK_API_VERSION_MAJOR(m_PhysicalDeviceProperties.apiVersion), VK_API_VERSION_MINOR(m_PhysicalDeviceProperties.apiVersion), VK_API_VERSION_PATCH(m_PhysicalDeviceProperties.apiVersion));
			SHAPE_LOG_INFO("GPU : {0}", m_DeviceInfo.Name);
			SHAPE_LOG_INFO("Memory : {0} mb", StringUtilities::ToString(m_DeviceInfo.Memory));
			SHAPE_LOG_INFO("Vendor : {0}", m_DeviceInfo.Vendor);
			SHAPE_LOG_INFO("Vendor ID : {0}", StringUtilities::ToString(m_DeviceInfo.VendorID));
			SHAPE_LOG_INFO("Device Type : {0}", std::string(PhysicalDeviceTypeToString(m_DeviceInfo.Type)));
			SHAPE_LOG_INFO("Driver Version : {0}", m_DeviceInfo.Driver);
			SHAPE_LOG_INFO("APi Version : {0}", m_DeviceInfo.APIVersion);
			...
			SHAPE_ASSERT(queueFamilyCount > 0, "");
			...
			if (extCount > 0)
			{
				...
				{
					SHAPE_LOG_INFO("Selected physical device has {0} extensions", extensions.size());
		...
		bool VKDevice::Init()
		{
			...
			{
				SHAPE_LOG_CRITICAL("[VULKAN] vkCreateDevice() failed!");
				return false;
			}
