...
namespace Shape
{
    namespace Maths
    {
        ...
        void Frustum::DefineOrtho(float scale, float aspectRatio, float gnear, float gfar, const glm::mat4& viewMatrix)
        {
            SHAPE_PROFILE_FUNCTION();
            glm::mat4 m = glm::ortho(-scale * aspectRatio, scale * aspectRatio, -scale, scale, gnear, gfar);
            m = m * viewMatrix;
            Define(m);
        }
        void Frustum::Define(float fov, float aspectRatio, float gnear, float gfar, const glm::mat4& viewMatrix)
        {
            SHAPE_PROFILE_FUNCTION();
            float tangent = tan(fov * 0.5f);
            float height = gnear * tangent;
            float width = height * aspectRatio;

            glm::mat4 m = glm::frustum(-width, width, -height, height, gnear, gfar);
            m = m * viewMatrix;
            Define(m);
        }
