...
namespace Shape
{
	namespace Graphics
	{
		...
		Mesh* CreateQuad()
		{
			...
			SharedPtr<Shape::Maths::BoundingBox> BoundingBox = CreateSharedPtr<Shape::Maths::BoundingBox>();
			for (int i = 0; i < 4; i++)
			{
				BoundingBox->Merge(data[i].Position);
			}
	...
	Graphics::Mesh* Graphics::CreatePrimative(PrimitiveType type)
	{
		...
		case Graphics::PrimitiveType::File:
			SHAPE_LOG_WARN("Trying to create primitive of type File");
			return nullptr;
		}

		SHAPE_LOG_ERROR("Primitive not supported");
		return nullptr;
	};
