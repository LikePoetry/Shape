...
namespace Shape::Graphics
{
    ...
    std::vector<Graphics::Mesh*> LoadMesh(tinygltf::Model& model, tinygltf::Mesh& mesh, std::vector<SharedPtr<Material>>& materials, Maths::Transform& parentTransform)
    {
        ...
        for (auto& primitive : mesh.primitives)
        {
            ...
            // -------- Indices ----------
            {
                ...
                {
                    SHAPE_LOG_WARN("Unsupported indices data type - {0}", componentTypeByteSize);
                }
            }
    ...
    void LoadNode(Model* mainModel, int nodeIndex, const glm::mat4& parentTransform, tinygltf::Model& model, std::vector<SharedPtr<Material>>& materials, std::vector<std::vector<Graphics::Mesh*>>& meshes)
    {
        ...
        SHAPE_LOG_INFO("asset.copyright : {0}", model.asset.copyright);
        SHAPE_LOG_INFO("asset.generator : {0}", model.asset.generator);
        SHAPE_LOG_INFO("asset.version : {0}", model.asset.version);
        SHAPE_LOG_INFO("asset.minVersion : {0}", model.asset.minVersion);
#endif
    ...
    void Model::LoadGLTF(const std::string& path)
    {
        ...
        if (!warn.empty())
        {
            SHAPE_LOG_ERROR(warn);
        }

        if (!ret)
        {
            SHAPE_LOG_ERROR("Failed to parse glTF");
        }
        {
            SHAPE_PROFILE_SCOPE("Parse GLTF Model");
            ...
            const tinygltf::Scene& gltfScene = model.scenes[Shape::Maths::Max(0, model.defaultScene)];
