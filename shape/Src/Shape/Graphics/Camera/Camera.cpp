...
namespace Shape
{
	...
	Maths::Ray Camera::GetScreenRay(float x, float y, const glm::mat4& viewMatrix, bool flipY) const
	{
		...
		glm::vec4 gnear = viewProjInverse * glm::vec4(x, y, 0.0f, 1.0f);
		gnear /= gnear.w;

		glm::vec4 gfar = viewProjInverse * glm::vec4(x, y, 1.0f, 1.0f);
		gfar /= gfar.w;

		ret.Origin = glm::vec3(gnear);
		ret.Direction = glm::normalize(glm::vec3(gfar) - ret.Origin);
