...
namespace Shape
{
	namespace Graphics
	{
		...
		{
			SHAPE_ASSERT(CreateFunc, "No Shader Create Function");
		...
		{
			SHAPE_ASSERT(CreateFuncFromEmbedded, "No Shader Create Function");
		...
		{
			SHAPE_ASSERT(CreateCompFuncFromEmbedded, "No Shader Create Function");
			return CreateCompFuncFromEmbedded(compData, compDataSize);
		}


		ShaderDataType Shader::SPIRVTypeToShapeDataType(const spirv_cross::SPIRType type)
		{
            ...
            SHAPE_LOG_WARN("Unknown spirv type!");
