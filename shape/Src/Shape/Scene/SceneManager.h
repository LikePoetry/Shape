...
namespace Shape
{
	...
	class SHAPE_EXPORT SceneManager
	{
	public:
		SceneManager();
		...
		// Jump to the scene index(stored in order they were originally added starting at zero)
		...
		void EnqueueScene(const std::string& name)
		{
			...
			SHAPE_LOG_INFO("[SceneManager] - Enqueued scene : {0}", name.c_str());
		}
